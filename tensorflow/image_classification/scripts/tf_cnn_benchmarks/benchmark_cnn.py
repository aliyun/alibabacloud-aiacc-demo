...
flags.DEFINE_boolean('fp16_gradient', False,
                     'whether cast to fp16 before allreduce')
...
flags.DEFINE_integer('image_size', 224, 'image size for model')
...
#   perseus: Distributed training using Perseus library. Runs workers using
#       an MPI framework (e.g. Open MPI). Each worker runs training on
#       single GPU, and averages gradients using NCCL or MPI all-reduce.
flags.DEFINE_enum('variable_update', 'parameter_server',
                  ('parameter_server', 'replicated', 'distributed_replicated',
                   ...
                   'collective_all_reduce', 'horovod', 'perseus'),
                  'The method for managing variables: parameter_server, '
                  'replicated, distributed_replicated, independent, '
                  'distributed_all_reduce, collective_all_reduce, horovod, perseus')
...
def create_config_proto(params):
  ...
  elif params.variable_update == 'perseus':
...
class BenchmarkCNN(object):
  ...
  def __init__(self, params, dataset=None, model=None):
    ...
    if self.params.variable_update in ['horovod', 'perseus'] and self.params.num_gpus > 1:
      raise ValueError('Horovod / Perseus benchmarks require num_gpus=1 on each worker')

    if self.params.variable_update in ['horovod', 'perseus'] and self.params.job_name:
      raise ValueError('job_name should not be specified for Horovod / Perseus.')
    ...
    elif self.params.variable_update in ('independent', 'horovod', 'perseus'):
      if self.job_name:
        raise ValueError('Invalid variable_update in distributed mode: %s' %
                         self.params.variable_update)
  ...
  def print_info(self):
    ...
    if self.params.variable_update in ['horovod', 'perseus'] and self.params.horovod_device:
  ...
  def _get_params_info(self):
    ...
    elif self.params.variable_update in ['horovod', 'perseus']:
  ...
  def _benchmark_graph(self, graph_info, eval_graph_info):
    ...
    sv = tf.train.Supervisor(
        ...
        # workers.
        is_chief=is_chief or (self.params.variable_update in ['horovod', 'perseus']
...
#    else:
  ...
  def _build_fetches(self, global_step, all_logits, losses, device_grads,
    ...
    if (self.params.variable_update in ['horovod', 'perseus'] or
  ...
  def add_forward_pass_and_gradients(self,
    ...
    def forward_pass_and_gradients():
      ...
      if self.params.variable_update == 'horovod':
        ...
        if self.params.fp16_gradient:
          grads = [hvd.allreduce(grad, average=True, compression=hvd.Compression.fp16) for grad in grads]
        else:
      ...
      if self.params.variable_update == 'perseus':
        ...
        # All-reduce gradients using Horovod.
        if self.params.fp16_gradient:
          grads = [tf.cast(hvd.allreduce(tf.cast(grad,dtype=tf.float16), average=True, device_dense=horovod_device), dtype=tf.float32)
                   for grad in grads]
        else:
          grads = [hvd.allreduce(grad, average=True, device_dense=horovod_device)
                   for grad in grads]
