...
#kv = perseus_kv.create("dist_sync_perseus") #args.kv_store)
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='coco', #'voc',
    ...
    parser.add_argument('--profiler', type=str, default='0',
                        help='Profiler of mxnet, 0: close, 1: open')

    # Norm layer options
    parser.add_argument('--norm-layer', type=str, default=None, choices=[None, 'bn', 'syncbn', 'perseussyncbn'],
    ...
    parser.add_argument('--kv-store', type=str, default='nccl',
                        help='KV store options. local, device, nccl, dist_sync, dist_device_sync, dist_sync_perseus'
...
def train(net, train_data, val_data, eval_metric, batch_size, ctx, args):
    """Training pipeline"""
    print("rank:{}, training...".format(kv.rank)) if "perseus" in args.kv_store else None
    ...
    if args.profiler == "1":
        ...
        profiler.set_config(profile_all=True,
                            ...
                            filename='profile_output_{}.json'.format(kv.rank if "perseus" in args.kv_store else 0)) 
    ...
    if args.horovod:
        ...
        trainer = gluon.Trainer(
            ...
            'sgd',
            optimizer_params,
            update_on_kvstore=None, kvstore=kv) #(False if args.amp else None), kvstore=kv)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        executor = Parallel(args.executor_threads, rcnn_task) if (not args.horovod and "perseus" not in args.kv_store) else None
        if args.mixup:
        ...
        for i, batch in enumerate(train_data):
            ...
            if ((not args.horovod) or hvd.rank() == 0) and args.log_interval \
                ...
                logger.info('rank:{}, [Epoch {}][Batch {}], Speed: {:.3f} samples/sec, {}'.format(
                    cur_rank, epoch, i, args.log_interval * batch_size / (time.time() - btic), msg))
                btic = time.time()
        ...
        if ((not args.horovod) and ("perseus" not in args.kv_store)) or (args.horovod and hvd.rank() == 0) or (("perseus" in args.kv_store) and kv.rank == 0): # perseus
            #if (not args.horovod) or hvd.rank() == 0:
...
if __name__ == '__main__':
    ...
    if args.custom_model:
        ...
        if args.norm_layer == 'syncbn':
        ...
        elif args.norm_layer == 'perseussyncbn':
            norm_layer = gluon.contrib.nn.PerseusSyncBatchNorm
            norm_kwargs = {'num_devices': len(ctx)}
            sym_norm_layer = mx.sym.contrib.PerseusSyncBatchNorm
            sym_norm_kwargs = {'ndev': len(ctx)}
            ...
            norm_kwargs = {} #None
            sym_norm_layer = mx.sym.BatchNorm #None
    ...
    batch_size = args.batch_size // num_gpus if (args.horovod or "perseus" in args.kv_store) else args.batch_size
    assert batch_size > 0
    print('rank:{}, args.batchsize:{}, batch_size:{}, num_gpus:{}'.format(kv.rank, args.batch_size, batch_size, num_gpus)) if "perseus" in args.kv_store else None
    ...
    #profiler.set_state('run')
    # training
    print(net) if "perseus" in args.kv_store and kv.rank == 0 else None
    ...
    # 2. stop recording
    #profiler.set_state('stop')
    # 3. dump all results to log file before download
    #print(profiler.dumps())
