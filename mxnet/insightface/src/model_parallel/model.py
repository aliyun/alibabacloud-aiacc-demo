...
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), 'model_parallel'))
from executor_manager import ModelParallelExecutorManager
...
def _create_kvstore(kvstore, num_device, arg_params):
    ...
    """
    update_on_kvstore = True #bool(int(os.getenv('MXNET_UPDATE_ON_KVSTORE', "1")))
    if kvstore is None:
        kv = None
...
def _initialize_modelparallel_kvstore(kvstore, param_arrays, arg_params, param_names, update_on_kvstore):
    """Initialize kvstore"""
    for idx, param_on_devs in enumerate(param_arrays):
        name = param_names[idx]
        kvstore.init(name, arg_params[name])
...
def exec_output_copyto(exec_m, output, index=0):
    for texec, islice in zip(exec_m.curr_execgrp.train_execs, exec_m.slices):
        texec.outputs[index].copyto(output[islice])

def excmngr_backward(exec_m, out_grads=None):
    curr_execgrp = exec_m.curr_execgrp
    for texec, islice in zip(curr_execgrp.train_execs, curr_execgrp.slices):
        texec.out_grads = [nd.empty(output.shape, ctx=output.context) for output in texec.outputs]
...
def _train_multi_device(symbol, ctx, args, arg_names, param_names, aux_names,
                        arg_params, aux_params,
                        fc_params,
                        begin_epoch, end_epoch, epoch_size, optimizer,
                        optimizer_fc,
                        kvstore, update_on_kvstore,
                        train_data, train_metric=None, eval_data=None, eval_metric=None,
    ...
    if logger is None:
        logger = logging
    ############################################################################
    # 1. DataParallel and ModelParallel init
    ############################################################################
    ...
    #model_parallel_executor.set_params(weight_param, bias_param)
    ...
    batchsize = args.per_batch_size * kvstore.num_workers #batchsize #train_data.shape[0]*kvstore.size #len(train_data) * kvstore.size                                               
    embedding = args.emb_size  
    #import pdb
    #pdb.set_trace()
    model_parallel_executor = ModelParallelExecutorManager(kvstore=kvstore,
                                                            ctx=ctx,
                                                            args=args,
                                                            batchsize=batchsize,
                                                            arg_names=arg_names,
    ...
    if kvstore:
        ...
        #fc_weight_params = nd.zeros(fc_params['weight'].shape)
        #fc_bias_params = nd.zeros(fc_params['bias'].shape)
        model_parallel_executor._initialize_kvstore()
        #model_parallel_executor.set_params(fc_weight_params, fc_bias_params)
    
    ##################################################################################
    # 2. Now start training
    ##################################################################################
    train_data.reset()
    batchsize = train_data.batch_size
    embedding = args.emb_size
    rank = kvstore.rank
    size = kvstore.num_workers
    ...
    for epoch in range(begin_epoch, end_epoch):
        ...
        while True:
            ...
            for data_batch in train_data:
                ...
                #eval_total_label = nd.empty((batchsize*kvstore.num_workers,))

                # forward
                feature_map = nd.empty((batchsize, embedding))
                total_features = nd.zeros((batchsize*kvstore.num_workers, embedding))
                total_labels = nd.zeros((batchsize*kvstore.num_workers, ))
                executor_manager.forward(is_train=True)
                exec_output_copyto(executor_manager, feature_map)

                # allreduce feature and label of multi gpus
                total_features[rank*batchsize: (rank+1)*batchsize] = feature_map
                total_labels[rank*batchsize: (rank+1)*batchsize] = data_batch.label[0]
                total_features = model_parallel_executor.allreduce('total_features', total_features)
                total_labels = model_parallel_executor.allreduce('total_labels', total_labels)
                
                #if rank == 0:
                #    import pdb

                # fc+softmax load data
                model_parallel_executor.load_data_batch(total_features, total_labels)
                # modelparallel forward
                model_parallel_executor.forward()
                # modelparallel backward
                model_parallel_executor.backward()
                # grad return to each gpu
                return_each_gpu_grad = model_parallel_executor.get_return_each_gpu_grad()
                
                excmngr_backward(executor_manager, return_each_gpu_grad)
                
                # update param
                model_parallel_executor.update_param()
                if update_on_kvstore:
                ...
                # loss
                if nbatch % 100 == 0:
                    loss = model_parallel_executor.get_return_each_gpu_loss()
                    loss_mean = loss.mean().asscalar()
                    return_each_gpu_grad = return_each_gpu_grad.abs().mean().asscalar()
                    
                    logging.info('Epoch[{0}] Worker:{1}, Batch:{2}, loss:{3:8.4f}, backbone-grad:{4:8.4f}'.format(epoch, rank, nbatch, loss_mean, return_each_gpu_grad))

                # evaluate at end, so we can lazy copy
                #executor_manager.update_metric(eval_metric, data_batch.label)
                # predict
                return_predict_value = model_parallel_executor.get_return_each_gpu_predict()
                train_metric.update(preds=return_predict_value, labels=total_labels) #eval_total_label)
                nbatch += 1
                # batch callback (for print purpose)
                if batch_end_callback is not None:
                    batch_end_params = BatchEndParam(epoch=epoch,
                                                     nbatch=nbatch,
                                                     eval_metric=train_metric, #eval_metric,
                                                     locals=locals())
...
class FeedForward(BASE_ESTIMATOR):
    ...
    def _init_params(self, inputs, classes, emb_size, overwrite=False):
        ...
        # fc_parameters init
        fc_params = {}
        fc_params['weight'] = nd.zeros((classes, emb_size))
        fc_params['bias'] = nd.zeros((classes))
        for k, v in fc_params.items():
            if self.fc_params and k in self.fc_params and (not overwrite):
                fc_params[k] = self.fc_param[k][:]
            else:
    ...
    def fit(self, args, X, y=None, eval_data=None, train_metric='acc', eval_metric='acc',
        ...
        arg_names, param_names, aux_names = \
                self._init_params(data.provide_data+data.provide_label, args.num_classes, args.emb_size)
        ...
        if isinstance(self.optimizer_fc, str):
            ...
            if kvstore and 'dist' in kvstore.type and '_async' not in kvstore.type:
                batch_size *= kvstore.num_workers
            optimizer_fc = opt.create(self.optimizer_fc,
                                   rescale_grad=(1.0/batch_size),
                                   **(self.kwargs))
        elif isinstance(self.optimizer_fc, opt.Optimizer):
            #if not optimizer.idx2name:
            #    optimizer.idx2name = param_idx2name.copy()
            optimizer_fc = self.optimizer_fc
        else:
            optimizer_fc = optimizer
        # do training
        _train_multi_device(self.symbol, self.ctx, 
                            args,
                            arg_names, param_names, aux_names,
    ...
    def create(symbol, X, y=None, ctx=None,
               ...
               eval_data=None, train_metric='acc', eval_metric='acc',
               epoch_end_callback=None, batch_end_callback=None,
        ...
        model.fit(X, y, eval_data=eval_data, train_metric=train_metric, eval_metric=eval_metric,
                  epoch_end_callback=epoch_end_callback,
                  batch_end_callback=batch_end_callback,
                  kvstore=kvstore,
                  logger=logger,
